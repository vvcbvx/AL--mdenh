   ...
   // انقل هذه المتغيرات خارج دالة initApp() لجعلها متاحة عالمياً
...
const restaurantDescriptionInput = document.getElementById('restaurant-description-input');
const saveRestaurantInfoBtn = document.getElementById('save-restaurant-info');
const restaurantTitle = document.querySelector('.title');
const restaurantSubtitle = document.querySelector('.subtitle');
...
// ... باقي المتغيرات ...
        ...
        function setupEventListeners() {
            ...
            // إضافة مستمع حدث لحفظ معلومات المطعم
saveRestaurantInfoBtn.addEventListener('click', saveRestaurantInfo);
            // Save Product
...
function createProductCard(product) {
  ...
  card.innerHTML = `
        ...
        <img src="${product.image || 'placeholder.jpg'}" 
             ...
             data-src="${product.image || 'placeholder.jpg' } loading="lazy">
        <div class="product-info">
            <h3 class="product-name">${product.name}</h3>
            <p class="product-description">${product.description}</p>
            <div class="product-price">
...
// Change Admin Password - الإصدار المصحح
async function changeAdminPassword() {
    const password = newPassword.value.trim();
    if (!password) {
        showToast('الرجاء إدخال كلمة مرور جديدة', 'error');
    ...
    try {
        ...
        // Update password - تأكد من استخدام المفتاح الصحيح
        currentData[config.jsonbin.passwordKey] = password;
        ...
        console.error('Error changing password:', error);
        showToast('فشل تغيير كلمة المرور: ' + error.message, 'error');
    } finally {
        hideLoading();
    }
}

// Load Restaurant Name from JSONBin.io
async function loadRestaurantName() {
    try {
        ...
        if (!response.ok) throw new Error('Failed to load restaurant name');
        
        const data = await response.json();
        const restaurantName = data.record[config.jsonbin.restaurantNameKey] || "مطعم المدينة والمندي";
        
        // تحديث اسم المطعم في الواجهة
        if (restaurantTitle) {
...
// Initialize the app
document.addEventListener('DOMContentLoaded', () => {
    loadRestaurantName(); // أضف هذا السطر
    loadProducts();
    setupEventListeners();
    loadCartFromLocalStorage();
    updateCartUI();
...
async function saveRestaurantInfo() {
    ...
    const description = restaurantDescriptionInput.value.trim();
    
    if (!name) {
        showToast('الرجاء إدخال اسم المطعم', 'error');
        return;
    }
    ...
    try {
        ...
        currentData.restaurantName = name;
        currentData.restaurantDescription = description;
        ...
        if (!updateResponse.ok) throw new Error('Failed to save restaurant info');
        ...
        showToast('تم حفظ معلومات المطعم بنجاح', 'success');
    } catch (error) {
        console.error('Error saving restaurant info:', error);
        showToast('فشل حفظ معلومات المطعم', 'error');
    } finally {
        hideLoading();
    }
}
...
// Save Restaurant Information - الإصدار المصحح
async function saveRestaurantInfo() {
    const name = restaurantNameInput.value.trim();
    const description = restaurantDescriptionInput.value.trim();
    ...
    try {
        ...
        // Update restaurant information - استخدام المفاتيح الصحيحة من config
        currentData[config.jsonbin.restaurantNameKey] = name;
        currentData.restaurantDescription = description;
        ...
        // Update UI - تحديث جميع العناصر التي تعرض اسم المطعم
        const titleElements = document.querySelectorAll('.title');
        titleElements.forEach(el => {
            if (el.id !== 'product-name') { // تجنب عناصر أخرى تحمل كلاس title
                el.textContent = name;
            }
        });
        
        if (description) {
            restaurantSubtitle.textContent = description;
        }
...
// Load Restaurant Name and Description from JSONBin.io
async function loadRestaurantInfo() {
    try {
        ...
        if (!response.ok) throw new Error('Failed to load restaurant info');
        
        const data = await response.json();
        const restaurantName = data.record[config.jsonbin.restaurantNameKey] || "مطعم المدينة والمندي";
        const restaurantDescription = data.record.restaurantDescription || "أشهى المأكولات بأجود المكونات";
        ...
        if (restaurantSubtitle) {
            restaurantSubtitle.textContent = restaurantDescription;
        }
        
        // ملء حقول الإدخال في لوحة الإدارة
        if (restaurantNameInput) {
            restaurantNameInput.value = restaurantName;
        }
        
        if (restaurantDescriptionInput) {
            restaurantDescriptionInput.value = restaurantDescription;
        }
    } catch (error) {
...
// Initialize the app
document.addEventListener('DOMContentLoaded', () => {
    loadRestaurantInfo(); // تحميل معلومات المطعم
    loadProducts();
    setupEventListeners();
    loadCartFromLocalStorage();
    updateCartUI();
...
async function saveRestaurantInfo() {
    ...
    try {
        // Configuration - إضافة config محلياً
        const config = {
            jsonbin: {
                binId: "6856a91e8960c979a5ae6570",
                apiKey: "$2a$10$DY6Bo8O3ePuyfWxRh63SFenGPrD/TXQru7k.//.isDVbR6giTbPKG",
